...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
    <a href="{{ route('admin.committees.index') }}" style="
        ...
        transition:0.3s;
    " onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">&larr; Back to Committees</a>
    ...
    <a href="{{ route('admin.committees.leadership.create', $committee->id) }}" style="
        ...
        text-decoration:none;
        transition:0.3s;
    " onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">+ Add Leader</a>
</div>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="padding:10px; border:1px solid #ddd; text-align:center;">
                ...
                @if($photoUrl)
                    <img src="{{ $photoUrl }}" alt="{{ $leader->name }}" style="width:60px; height:60px; object-fit:cover; border-radius:50%;">
                @else
            ...
            <td style="padding:10px; border:1px solid #ddd;">
                <a href="{{ route('admin.committees.leadership.edit', [$committee->id, $leader->id]) }}" style="margin-right:5px; color:#fff; background:#FFC107; padding:4px 8px; border-radius:4px; text-decoration:none;">Edit</a>
                <form action="{{ route('admin.committees.leadership.destroy', [$committee->id, $leader->id]) }}" method="POST" style="display:inline-block;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" onclick="return confirm('Delete this leader?');" style="color:#fff; background:#F44336; padding:4px 8px; border:none; border-radius:4px; cursor:pointer;">Delete</button>
                </form>
            </td>
        </tr>
        @empty
        <tr>
            <td colspan="6" style="padding:10px; text-align:center; color:#888;">No leaders assigned yet.</td>
        </tr>
        @endforelse
    </tbody>
</table>
